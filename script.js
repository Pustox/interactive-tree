...
navigator.mediaDevices.getUserMedia({ audio: true }).then((stream) => {
  ...
  function animateTree() {
    if (!isTreeLit) { // Ёлка мигает, только если не сказано "гори"
      ...
      if (volume > 30) treeLevels[1].style.opacity = '1';
      if (volume > 40) treeLevels[2].style.opacity = '1';
      if (volume > 50) star.style.opacity = '1';

      // Если тише, то гасим
      if (volume < 50) star.style.opacity = '0.2';
      if (volume < 40) treeLevels[2].style.opacity = '0.2';
      if (volume < 30) treeLevels[1].style.opacity = '0.2';
      if (volume < 20) treeLevels[0].style.opacity = '0.2';
    }
